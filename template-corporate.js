// Corporate Template Renderer - Enhanced with Drag & Drop, Inline Editing & Image Support
function renderCorporateTemplate(data) {
  const personal = data.personal || {};
  const skillsArr = (data.skills ? data.skills.split(',').map(s => s.trim()).filter(Boolean) : []);
  
  let html = `<div class="template-corporate" id="corporateCanvas">`;
  
  html += `
    <div class="draggable" data-type="name" style="top:45px;left:55px;">
      <h1 contenteditable="false">${escapeHtml(personal.fullName || "Your Name")}</h1>
    </div>
    
    <div class="draggable" data-type="title" style="top:105px;left:55px;">
      <div style="font-weight: 500; font-size:1.1rem; color: #1e3a2f;">${escapeHtml(personal.jobTitle || "Professional Title")}</div>
    </div>
    
    <div class="draggable" data-type="contact" style="top:145px;left:55px;">
  ...
  // Education
  html += `<div class="draggable section-block" data-type="education" style="top:230px;left:55px;width:80%;">`;
  ...
  // Work Experience
  html += `<div class="draggable section-block" data-type="work" style="top:390px;left:55px;width:80%;">`;
  ...
  // Skills
  html += `<div class="draggable section-block" data-type="skills" style="top:620px;left:55px;width:80%;">`;
  ...
  // Projects
  html += `<div class="draggable section-block" data-type="projects" style="top:730px;left:55px;width:80%;">`;
    ...
    data.projects.forEach((proj, i) => {
      html += `
        <div class="project-item" data-idx="${i}">
          ...
          ${proj.link ? `<br/><a href="${escapeHtml(proj.link)}" target="_blank" style="font-size:0.8rem;color:#1e3a2f;">${escapeHtml(proj.link)}</a>` : ''}
        </div>`;
    });
  }
  html += `</div>`;

  html += `</div>`; // end template-corporate
  return html;
}