...
(function() {
    ...
    function renderCorporateCleanTemplate() {
        ...
        if (!leftSidebarEl || !rightMainEl) return;
    ...
    function refreshCorporateEditorFields() {
        ...
        activeState.corporateSectionsList.forEach((sec, sIdx) => {
            ...
            if (sec.type === 'contact') {
                ...
                bodyHTML += `<textarea rows="4" style="width:100%; font-size:11px; font-family:inherit; padding:6px; border:1px solid #cbd5e1; border-radius:4px; resize:vertical;" oninput="window.state.corporateSectionsList[${sIdx}].desc=this.value; window.renderCorporateCleanTemplate();">${sec.desc || ''}</textarea>`;
            } else if (sec.type === 'experience') {
                sec.items.forEach((item, itemIdx) => {
                    bodyHTML += `
                        <div style="background:#f8fafc; padding:6px; border-radius:4px; border:1px solid #cbd5e1; margin-bottom:4px; position:relative;">
    ...
    window.corpSwitchMobileTab = function(tab) {
        const editorEl = document.getElementById('corp-editor-panel');
        const previewEl = document.getElementById('corp-preview-panel');
        const btnEditor = document.getElementById('corp-tab-editor');
        const btnPreview = document.getElementById('corp-tab-preview');
...
// ============================================================
// PDF DOWNLOAD FOR CORPORATE CLEAN TEMPLATE
// ============================================================
function downloadCorporateCleanPDF() {
    console.log("📥 [Corporate] Downloading PDF...");
    
    const element = document.getElementById('corporate-paper');
    if (!element) {
    ...
    clone.querySelectorAll('.builder-editor, .editor-panel, .editor-controls, form, input, button, select, textarea, .no-print, .corporate-editor-panel, .corp-mobile-tabs, .editor-header, .editor-group-card, .avatar-hover-overlay')
    ...
    // Fix top accent bar
    const accent = clone.querySelector('.top-accent-bar');
    if (accent) {
    ...
    // Fix template grid
    const grid = clone.querySelector('.template-grid');
    if (grid) {
        grid.style.cssText = `
            display: flex !important;
            flex: 1 !important;
    ...
    // Fix left column
    const left = clone.querySelector('.left-column');
    if (left) {
        left.style.cssText = `
    ...
    // Fix right column
    const right = clone.querySelector('.right-column');
    if (right) {
        right.style.cssText = `
            flex: 1 !important;
    ...
    const divider = clone.querySelector('.vertical-divider');
    if (divider) {
    ...
    // Fix header text
    const h1 = clone.querySelector('.header-text-block h1');
    if (h1) {
        h1.style.cssText = `
    ...
    const role = clone.querySelector('.header-text-block div');
    if (role) {
        role.style.cssText = `
            margin: 0 !important;
    ...
    // Fix avatar
    const avatar = clone.querySelector('.avatar-container');
    if (avatar) {
    ...
    // Fix side title
    clone.querySelectorAll('.side-title').forEach(el => {
        el.style.cssText = `
            font-weight: 800 !important;
            color: #000000 !important;
    ...
    // Fix contact items
    clone.querySelectorAll('.contact-item').forEach(el => {
        el.style.cssText = `
            display: flex !important;
    ...
    const opt = {
        margin: 0,
        filename: `${userName}_CV_Corporate.pdf`,
    ...
    html2pdf()
        ...
        .from(clone)
        .save()
        .then(() => console.log("✅ [Corporate] PDF generated successfully!"))
        .catch(err => {
            console.error("❌ [Corporate] PDF Error:", err);
...
window.downloadCorporateCleanPDF = downloadCorporateCleanPDF;