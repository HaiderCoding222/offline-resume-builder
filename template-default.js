(function () {
    ...
    // Dynamic Viewport Scaler for Mobile Screens
    function adjustCVScale() {
        ...
        if (!previewPanel || !paperWrapper) return;

        if (window.innerWidth <= 820) {
            const containerWidth = previewPanel.clientWidth - 20; // 20px padding
            const baseWidth = 794; // Fixed Desktop Paper Width
            const scale = Math.min(containerWidth / baseWidth, 1);
            
            paperWrapper.style.transform = `scale(${scale})`;
            paperWrapper.style.marginBottom = `-${(1123 - (1123 * scale))}px`; // Prevent extra bottom whitespace
        } else {
            paperWrapper.style.transform = 'scale(1)';
            paperWrapper.style.marginBottom = '0px';
        }
    }
    ...
    function injectMobileTabs() {
        ...
        const parentPane = editorContainer.closest('.left-editor-panel') || editorContainer.parentElement;
    ...
    window.tmplSwitchTab = function(activeTab) {
        const editorPane = document.querySelector('.left-editor-panel');
        const previewPane = document.querySelector('.right-preview-panel');
        ...
        if (window.innerWidth <= 820) {
            if (activeTab === 'editor') {
                ...
                setTimeout(adjustCVScale, 50); // Scale recalculate on tab switch
            }
        }
    };
                ...
                templateState.skills.forEach((sk, i) => {
                    itemsHTML += `
                        <div style="display:flex; gap:6px; margin-bottom:6px; align-items:center; flex-wrap:wrap;">
                            ...
                            <button onclick="window.tmplCloneItem('skills', ${i})" style="padding:6px 10px; font-size:12px; cursor:pointer;" title="Clone Skill">📋</button>
                            <button onclick="window.tmplDeleteItem('skills', ${i})" style="padding:6px 10px; font-size:12px; color:red; cursor:pointer;" title="Delete Skill">🗑️</button>
                        </div>
                    `;
                });
                    ...
                    itemsHTML += `
                        <div style="border:1px dashed #cbd5e1; padding:8px; margin-bottom:8px; border-radius:4px; background:#fff;">
                            <div style="display:flex; justify-content:space-between; margin-bottom:6px; flex-wrap:wrap; gap:4px;">
                                ...
                                <div>
                                    <button onclick="window.tmplCloneItem('experience', ${i})" style="padding:4px 8px; font-size:12px; cursor:pointer;" title="Clone Experience">📋 Clone</button>
                                    <button onclick="window.tmplDeleteItem('experience', ${i})" style="padding:4px 8px; font-size:12px; color:red; cursor:pointer;" title="Delete Experience">🗑️</button>
                                </div>
                            </div>
                    ...
                    itemsHTML += `
                        <div style="border:1px dashed #cbd5e1; padding:8px; margin-bottom:8px; border-radius:4px; background:#fff;">
                            <div style="display:flex; justify-content:space-between; margin-bottom:6px; flex-wrap:wrap; gap:4px;">
                                ...
                                <div>
                                    <button onclick="window.tmplCloneItem('education', ${i})" style="padding:4px 8px; font-size:12px; cursor:pointer;" title="Clone Education">📋 Clone</button>
                                    <button onclick="window.tmplDeleteItem('education', ${i})" style="padding:4px 8px; font-size:12px; color:red; cursor:pointer;" title="Delete Education">🗑️</button>
                                </div>
                            </div>
    ...
    function downloadPDF() {
        const element = document.querySelector(".paper-wrapper");
        if (!element) {
        ...
        // Reset scale momentarily for 100% crisp PDF export
