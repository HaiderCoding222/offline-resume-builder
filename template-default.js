...
(function () {
    ...
    window.templateState = JSON.parse(JSON.stringify(defaultData));
...
// ============================================================
// PDF DOWNLOAD FOR DEFAULT TEMPLATE
// ============================================================
function downloadDefaultPDF() {
    console.log("📥 [Default] Downloading PDF...");
    
    const element = document.querySelector('.cv-premium-container');
    if (!element) {
    ...
    clone.querySelectorAll('.builder-editor, .editor-panel, .editor-controls, form, input, button, select, textarea, .no-print, .left-editor-panel')
    ...
    // Fix main content
    const main = clone.querySelector('.cv-main-content');
    if (main) {
        main.style.cssText = `
            width: 68% !important;
    ...
    // Fix avatar
    const avatar = clone.querySelector('.cv-avatar-placeholder');
    if (avatar) {
        avatar.style.cssText = `
            ...
            border: 4px solid #4f5d73 !important;
        `;
    }

    // Fix sidebar headings
    clone.querySelectorAll('.sidebar-heading').forEach(el => {
        el.style.cssText = `
            font-size: 13.5px !important;
            font-weight: 700 !important;
            letter-spacing: 1px !important;
            border-bottom: 1px solid #4f5d73 !important;
    ...
    // Fix sidebar text
    clone.querySelectorAll('.sidebar-text, .sidebar-skill-bullet').forEach(el => {
        el.style.cssText = `
    ...
    // Fix main headings
    clone.querySelectorAll('.main-section-heading').forEach(el => {
        el.style.cssText = `
    ...
    // Fix name
    const nameEl = clone.querySelector('#tmpl-name, .cv-header-block h1');
    if (nameEl) {
    ...
    const userName = window.templateState.resumeData.name 
    ? window.templateState.resumeData.name.trim().replace(/\s+/g, '_') 
    : 'Resume';

    const opt = {
        margin: 0,
        filename: `${userName}_CV_Default.pdf`,
    ...
    html2pdf()
        ...
        .from(clone)
        .save()
        .then(() => console.log("✅ [Default] PDF generated successfully!"))
        .catch(err => {
            console.error("❌ [Default] PDF Error:", err);
...
window.downloadDefaultPDF = downloadDefaultPDF;