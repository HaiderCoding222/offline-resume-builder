// ====================================================
// TEMPLATE DEFAULT - SELF CONTAINED EDITOR, PREVIEW, RESPONSIVE TABS & PDF
// ====================================================
...
(function () {
    ...
    // --- DYNAMIC MOBILE TABS INJECTION --- //
    function injectMobileTabs() {
        if (document.getElementById('tmpl-mobile-tabs')) return;

        const editorContainer = document.getElementById('tmpl-dynamic-editor-sections');
        ...
        // Insert before editor panel
        const parentPane = editorContainer.closest('.editor-panel-full') || editorContainer.parentElement;
        if (parentPane && parentPane.parentElement) {
            parentPane.parentElement.insertBefore(tabsBar, parentPane);
        }
    }

    window.tmplSwitchTab = function(activeTab) {
        const editorPane = document.getElementById('tmpl-dynamic-editor-sections')?.closest('.editor-panel-full') || document.getElementById('tmpl-dynamic-editor-sections')?.parentElement;
        const previewPane = document.querySelector('.preview-panel-full') || document.querySelector('.paper-wrapper')?.parentElement;
        ...
        if (window.innerWidth <= 768) {
            if (activeTab === 'editor') {
                if (editorPane) editorPane.style.setProperty('display', 'block', 'important');
                if (previewPane) previewPane.style.setProperty('display', 'none', 'important');
                btnEditor?.classList.add('active');
                btnPreview?.classList.remove('active');
            } else {
                if (editorPane) editorPane.style.setProperty('display', 'none', 'important');
                if (previewPane) previewPane.style.setProperty('display', 'block', 'important');
            ...
            if (sec.type === 'profile') {
                ...
                templateState.skills.forEach((sk, i) => {
                    itemsHTML += `
                        <div style="display:flex; gap:4px; margin-bottom:5px; align-items:center;">
                            <input type="text" value="${sk.name}" placeholder="Skill Name" oninput="window.tmplUpdateItem('skills', ${i}, 'name', this.value)" style="flex:1; font-size:11px; padding:3px;">
                            <button onclick="window.tmplCloneItem('skills', ${i})" style="padding:2px 5px; font-size:10px; cursor:pointer;" title="Clone Skill">📋</button>
                            <button onclick="window.tmplDeleteItem('skills', ${i})" style="padding:2px 5px; font-size:10px; color:red; cursor:pointer;" title="Delete Skill">🗑️</button>
                        </div>
                    `;
                });
                itemsHTML += `<button onclick="window.tmplAddItem('skills')" style="width:100%; font-size:11px; padding:4px; margin-top:5px; cursor:pointer; background:#e2e8f0; border:1px solid #cbd5e1; border-radius:3px;">+ Add Skill</button>`;
            }
            else if (sec.type === 'experience') {
                    ...
                    itemsHTML += `
                        <div style="border:1px dashed #cbd5e1; padding:6px; margin-bottom:6px; border-radius:4px; background:#fff;">
                            <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                                <strong style="font-size:10px; color:#475569;">Experience #${i + 1}</strong>
                                <div>
                                    <button onclick="window.tmplCloneItem('experience', ${i})" style="padding:1px 4px; font-size:10px; cursor:pointer;" title="Clone Experience">📋 Clone</button>
                                    <button onclick="window.tmplDeleteItem('experience', ${i})" style="padding:1px 4px; font-size:10px; color:red; cursor:pointer;" title="Delete Experience">🗑️</button>
                                </div>
                            </div>
                            <input type="text" value="${exp.company || ''}" placeholder="Company" oninput="window.tmplUpdateItem('experience', ${i}, 'company', this.value)" style="width:100%; font-size:11px; padding:3px; margin-bottom:3px;">
                            <input type="text" value="${exp.role || ''}" placeholder="Role / Designation" oninput="window.tmplUpdateItem('experience', ${i}, 'role', this.value)" style="width:100%; font-size:11px; padding:3px; margin-bottom:3px;">
                            <input type="text" value="${exp.duration || ''}" placeholder="Duration" oninput="window.tmplUpdateItem('experience', ${i}, 'duration', this.value)" style="width:100%; font-size:11px; padding:3px; margin-bottom:3px;">
                            <textarea rows="2" placeholder="Responsibilities" oninput="window.tmplUpdateItem('experience', ${i}, 'details', this.value)" style="width:100%; font-size:11px; padding:3px;">${exp.details || ''}</textarea>
                        </div>
                    `;
                });
                itemsHTML += `<button onclick="window.tmplAddItem('experience')" style="width:100%; font-size:11px; padding:4px; cursor:pointer; background:#e2e8f0; border:1px solid #cbd5e1; border-radius:3px;">+ Add Experience</button>`;
            }
            else if (sec.type === 'education') {
                templateState.education.forEach((edu, i) => {
                    itemsHTML += `
                        <div style="border:1px dashed #cbd5e1; padding:6px; margin-bottom:6px; border-radius:4px; background:#fff;">
                            <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                                <strong style="font-size:10px; color:#475569;">Education #${i + 1}</strong>
                                <div>
                                    <button onclick="window.tmplCloneItem('education', ${i})" style="padding:1px 4px; font-size:10px; cursor:pointer;" title="Clone Education">📋 Clone</button>
                                    <button onclick="window.tmplDeleteItem('education', ${i})" style="padding:1px 4px; font-size:10px; color:red; cursor:pointer;" title="Delete Education">🗑️</button>
                                </div>
                            </div>
                            <input type="text" value="${edu.institute || ''}" placeholder="Institute" oninput="window.tmplUpdateItem('education', ${i}, 'institute', this.value)" style="width:100%; font-size:11px; padding:3px; margin-bottom:3px;">
                            <input type="text" value="${edu.degree || ''}" placeholder="Degree" oninput="window.tmplUpdateItem('education', ${i}, 'degree', this.value)" style="width:100%; font-size:11px; padding:3px; margin-bottom:3px;">
                            <input type="text" value="${edu.year || ''}" placeholder="Year" oninput="window.tmplUpdateItem('education', ${i}, 'year', this.value)" style="width:100%; font-size:11px; padding:3px;">
                        </div>
                    `;
                });
                itemsHTML += `<button onclick="window.tmplAddItem('education')" style="width:100%; font-size:11px; padding:4px; cursor:pointer; background:#e2e8f0; border:1px solid #cbd5e1; border-radius:3px;">+ Add Education</button>`;
            }
    ...
    window.renderDefaultTemplate = function () {
        ...
        // Default tab view state check
        if (window.innerWidth <= 768) {
            window.tmplSwitchTab('editor');
        }
    };

    // Auto Init & Attach Download Listener
    setTimeout(() => {
