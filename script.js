...
document.getElementById("addImageBtn").addEventListener('click', () => {
  const canvas = document.querySelector('.template-modern, .template-creative, .template-corporate, .template-classic');
  if (!canvas) {
    showToast("Please select Modern template first");
    return;
  }
 
  const imgContainer = document.createElement('div');
  imgContainer.className = 'draggable image-container';
  imgContainer.style.width = '160px';
  imgContainer.style.height = '160px';
  imgContainer.style.top = '80px';
  imgContainer.style.left = '620px';
  imgContainer.innerHTML = `
    <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:#64748b;font-size:0.8rem;text-align:center;">
  ...
  makeImageInteractive(imgContainer);
 
  showToast("🖼️ Image container added - click to upload");
});

function makeImageInteractive(container) {
  container.addEventListener('click', () => {
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (ev) => {
        container.innerHTML = `<img src="${ev.target.result}" alt="Profile">`;
  ...
  // Right click to toggle circle / square shape
  container.addEventListener('contextmenu', (e) => {
    e.preventDefault();
    container.classList.toggle('circle');
    showToast("Shape toggled (Circle ↔ Rectangle)");
  });
}
...
function attachInteractiveListeners() {
  ...
  // Make all draggable elements interactive
...
document.addEventListener('mousemove', (e) => {
  ...
  const canvas = currentDraggedElement.parentElement;
  ...
  // Bounds checking
  newLeft = Math.max(20, Math.min(newLeft, canvasRect.width - currentDraggedElement.offsetWidth - 20));
  newTop = Math.max(20, Math.min(newTop, canvasRect.height - currentDraggedElement.offsetHeight - 20));
...
document.addEventListener('mouseup', () => {
  if (currentDraggedElement) {
    ...
    // Optionally save positions to resumeData.customPositions if you want persistence
  }
});

// Inline editing
function makeInlineEditable(element) {
  const editables = element.querySelectorAll('h1, div, span, strong, p, .skill-tag, .job-title');
  editables.forEach(el => {
    ...
    el.addEventListener('blur', () => {
      // You can add logic here to sync back to resumeData if needed
      // For full sync it's complex - this gives immediate visual feedback
      showToast("✓ Change saved visually (full data sync in future)");
    });
  });
}
