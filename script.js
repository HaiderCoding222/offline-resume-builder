 // ---------- Data Models ----------
  ...
  // ---- Preview generation based on template ----
  function updatePreview() {
    ...
    resumeData.work.forEach(work => {
      html += `<div class="job-item"><div class="job-title">${escapeHtml(work.title)} at ${escapeHtml(work.company)}</div><div class="job-dates">${escapeHtml(work.startDate)} — ${escapeHtml(work.endDate)}</div><div>${escapeHtml(work.description || '')}</div></div>`;
    });
    // Skills
    html += `<div class="section-title-resume">⚙️ Skills</div><div class="skills-list">`;
    skillsArr.forEach(skill => { if(skill) html += `<span class="skill-tag">${escapeHtml(skill)}</span>`; });
    html += `</div>`;
    // Projects
    html += `<div class="section-title-resume">📁 Projects</div>`;
    resumeData.projects.forEach(proj => {
      html += `<div class="project-item"><strong>${escapeHtml(proj.name)}</strong><br/>${escapeHtml(proj.description)}${proj.link ? `<br/><a href="#" style="font-size:0.7rem;">${escapeHtml(proj.link)}</a>` : ''}</div>`;
    });
    html += `</div>`;
