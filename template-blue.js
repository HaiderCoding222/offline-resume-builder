...
/* ============================================================ */
function switchBlueTab(tabName) {
    const editorPanel = document.getElementById('blue-editor-panel');
    const previewPanel = document.getElementById('blue-preview-panel');
    const btnEdit = document.getElementById('btn-blue-edit');
    const btnPreview = document.getElementById('btn-blue-preview');
...
window.switchBlueTab = switchBlueTab;

function downloadBluePDF() {
    console.log("📥 [Blue] Downloading PDF...");
    
    const element = document.querySelector('.cv-blue-content');
    if (!element) {
    ...
    if (typeof html2pdf === 'undefined') {
        alert("❌ html2pdf library not loaded! Check your script tag.");
        return;
    }

    const clone = element.cloneNode(true);
    
    clone.querySelectorAll('.builder-editor, .editor-panel, .editor-controls, form, input, button, select, textarea, .no-print, .blue-editor')
    ...
    // Fix header
    const header = clone.querySelector('.cv-blue-header');
    if (header) {
    ...
    // Fix body
    const body = clone.querySelector('.cv-blue-body');
    if (body) {
        body.style.cssText = `
            display: flex !important;
            flex: 1 !important;
    ...
    // Fix main content
    const main = clone.querySelector('.main-content');
    if (main) {
        main.style.cssText = `
            flex: 2 !important;
    ...
    const opt = {
        margin: 0,
        filename: `${userName}_CV_Blue.pdf`,
    ...
    html2pdf()
        ...
        .from(clone)
        .save()
        .then(() => console.log("✅ [Blue] PDF generated successfully!"))
        .catch(err => {
            console.error("❌ [Blue] PDF Error:", err);
