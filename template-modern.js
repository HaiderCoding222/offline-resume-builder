function renderModernTemplate(state) {
    ...
    if (avatarEl) {
        if (state.resumeData.photo) {
            ...
            avatarEl.style.backgroundImage = 'linear-gradient(135deg, #cbd5e0, #718096)';
        }
    }
    ...
    // Skills (Modern style overrides bullet to text)
            ...
            if (item.title && item.value && item.value.trim()) {
                csContainer.insertAdjacentHTML('beforeend', `
                    <div class="modern-section">
                        <h3 class="modern-heading">${item.title.toUpperCase()}</h3>
                        <p class="modern-text" style="white-space: pre-line;">${item.value}</p>
                    </div>
                `);
            }
        });
