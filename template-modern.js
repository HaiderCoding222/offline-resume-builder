...
// PDF Downloader for Modern Layout - FIXED
function downloadModernPDF() {
    ...
    clone.querySelectorAll('.builder-editor, .editor-panel, .editor-controls, form, input, button, select, textarea, .no-print, .modern-editor, .tmpl-mobile-tabs-container')
        .forEach(el => el.remove());
    
    // Set clean styles on clone - Force A4 exact dimensions
    clone.style.cssText = `
    ...
    clone.querySelectorAll('.ats-section-heading').forEach(el => {
        el.style.cssText = `
    ...
    // Fix all horizontal rules
    clone.querySelectorAll('.ats-hr').forEach(el => {
        el.style.cssText = `
    ...
    // Fix all row splits
    clone.querySelectorAll('.ats-row-split').forEach(el => {
        el.style.cssText = `
    ...
    // Fix all details boxes
    clone.querySelectorAll('.ats-details-box').forEach(el => {
        el.style.cssText = `
            font-size: 11px !important;
    ...
    // Fix header
    const header = clone.querySelector('.ats-header');
    if (header) {
        ...
        const contactRow = header.querySelector('.ats-contact-row');
        if (contactRow) {
    ...
    // Fix all sections
    clone.querySelectorAll('.ats-section').forEach(el => {
        el.style.cssText = `
            margin-bottom: 12px !important;
    ...
    const userName = (window.state && window.state.resumeData && window.state.resumeData.name) 
        ? window.state.resumeData.name.trim().replace(/\s+/g, '_') 
        : 'Resume';

    const opt = {
        margin: 0,
        filename: `${userName}_CV_Modern.pdf`,
    ...
    html2pdf()
        ...
        .save()
        .catch(err => {
            console.error("Modern PDF Error:", err);
...
// Override the download button for Modern template
