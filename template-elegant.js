...
function downloadElegantPDF() {
    ...
    // Fix section titles
    clone.querySelectorAll('.elegant-section-title').forEach(el => {
        el.style.cssText = `
    ...
    // Fix name
    const nameEl = clone.querySelector('.elegant-name-block h1');
    if (nameEl) {
    ...
    // Fix profession
    const profEl = clone.querySelector('.elegant-profession');
    if (profEl) {
        profEl.style.cssText = `
    ...
    // Fix avatar
    const avatar = clone.querySelector('.elegant-avatar');
    if (avatar) {
    ...
    const userName = state.elegantData?.name 
        ? state.elegantData.name.trim().replace(/\s+/g, '_') 
        : 'Resume';
