function renderElegantTemplate(state) {
    ...
    if (printNode) printNode.innerHTML = previewHTML;
...
function switchElegantTab(tabName) {
    const editorPanel = document.getElementById('elegant-editor-panel');
    const previewPanel = document.getElementById('elegant-preview-panel');
    const btnEdit = document.getElementById('btn-elegant-edit');
    const btnPreview = document.getElementById('btn-elegant-preview');

    if (!editorPanel || !previewPanel) return;

    if (tabName === 'edit') {
        editorPanel.classList.remove('hidden-panel');
        previewPanel.classList.remove('active-panel');
        if (btnEdit) btnEdit.classList.add('active');
        if (btnPreview) btnPreview.classList.remove('active');
    } else if (tabName === 'preview') {
        editorPanel.classList.add('hidden-panel');
...
window.switchElegantTab = switchElegantTab;

// ============================================================
// PDF DOWNLOAD FOR ELEGANT TEMPLATE
// ============================================================
function downloadElegantPDF() {
    console.log("📥 [Elegant] Downloading PDF...");
    
    const element = document.querySelector('.elegant-theme');
    if (!element) {
    ...
    clone.querySelectorAll('.builder-editor, .editor-panel, .editor-controls, form, input, button, select, textarea, .no-print, .elegant-editor-sidebar')
    ...
    const header = clone.querySelector('.elegant-header');
    if (header) {
    ...
    const body = clone.querySelector('.elegant-body');
    if (body) {
        body.style.cssText = `
            display: flex !important;
    ...
    const left = clone.querySelector('.elegant-left-column');
    if (left) left.style.cssText = `width: 38% !important;`;

    const right = clone.querySelector('.elegant-right-column');
    if (right) right.style.cssText = `width: 62% !important;`;
    ...
    const opt = {
        ...
        jsPDF: { unit: 'px', format: [794, 1110], orientation: 'portrait' }
    };
    ...
    html2pdf()
        ...
        .from(clone)
        .save()
        .then(() => console.log("✅ [Elegant] PDF generated successfully!"))
        .catch(err => {
            console.error("❌ [Elegant] PDF Error:", err);
            alert("PDF Download mein problem aayi: " + err.message);
        })
        .finally(() => {
