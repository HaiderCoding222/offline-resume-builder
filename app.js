...
async function loadActiveTemplate() {
    ...
    try {
        ...
        const container = placeholder.querySelector('.cv-premium-container') || 
                 placeholder.querySelector('.page');
        if (container) {
            container.style.width = '100%';
            container.style.maxWidth = container.classList.contains('page') ? '850px' : '794px';
...
function updatePreviewRender() {
    ...
    } else if (state.currentTemplate === 'blue' && typeof renderBlueTemplate === 'function') {
        renderBlueTemplate(state);
    } else if (state.currentTemplate === 'teal' && typeof renderTealTemplate === 'function') {
    renderTealTemplate(state);
    }else if (state.currentTemplate === 'corporate-clean' && typeof renderCorporateCleanTemplate === 'function') {
        renderCorporateCleanTemplate(state);
    }
}
...
btnDownloadPDF.addEventListener('click', async () => {
    ...
    const cvElement = placeholder.querySelector('.cv-teal-container') || 
                  placeholder.querySelector('.cv-premium-container') || 
                  placeholder.querySelector('.cv-ats-theme') || 
                  placeholder.querySelector('.cv-blue-container') ||
                  placeholder.querySelector('.page');
    ...
    const mobileScale = isMobile ? 1.5 : 2;
    ...
    const opt = {
        ...
        html2canvas: { 
            ...
            onclone: function(clonedDoc) {
                const tealEl = clonedDoc.querySelector('.cv-teal-container');
                const otherEl = clonedDoc.querySelector('.cv-premium-container') || 
                                clonedDoc.querySelector('.cv-ats-theme') || 
                                clonedDoc.querySelector('.cv-blue-container');
                                
                if (tealEl) {
                    tealEl.style.width = '794px';
                    tealEl.style.maxWidth = '794px';
                    tealEl.style.minWidth = '794px';
                    tealEl.style.transform = 'none';
                    tealEl.style.zoom = '1';
                    tealEl.style.margin = '0';
                    tealEl.style.boxShadow = 'none';
                    
                    clonedDoc.documentElement.style.width = '794px';
                    clonedDoc.body.style.width = '794px';
                    clonedDoc.body.style.maxWidth = '794px';
                } else if (otherEl) {
                    otherEl.style.width = '1100px';
                    otherEl.style.maxWidth = '1100px';
                    otherEl.style.minWidth = '1100px';
                    otherEl.style.transform = 'none';
                    otherEl.style.zoom = '1';
                    otherEl.style.margin = '0';
                    otherEl.style.boxShadow = 'none';
                    otherEl.style.position = 'relative';
                    otherEl.style.left = '0';
                    otherEl.style.top = '0';

                    clonedDoc.documentElement.style.width = '1100px';
                    clonedDoc.body.style.width = '1100px';
                    clonedDoc.body.style.maxWidth = '1100px';
                }
        ...
        pagebreak: { 
            mode: ['avoid-all', 'css'],
            avoid: ['.cv-experience-node', '.sidebar-section', '.ats-section', '.cv-blue-section']
        }
    };
    ...
    try {
        ...
        if (isMobile) {
            if (confirm('Direct download fail hua. Kya aap Print option se PDF save karna chahte hain?')) {
            ...
            alert("PDF generate nahi hui. Dobara try karo.");
        }
    } finally {
