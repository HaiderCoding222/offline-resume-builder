...
btnDownloadPDF.addEventListener('click', async () => {
    ...
    if (!placeholder || !placeholder.children.length) {
        alert("Pehle resume preview load hone do!");
        return;
    }

    // Direct CV container ko target karo (blank pages avoid karne ke liye)
    let cvElement = placeholder.querySelector('.cv-premium-container');

    if (!cvElement) {
        alert("CV template nahi mila. Page refresh karke try karo.");
        return;
    }
    ...
    const opt = {
        margin: [8, 8, 8, 8],
        filename: `${(state.resumeData.name || 'My_CV').replace(/[^a-zA-Z0-9]/g, '_')}_Resume.pdf`,
    ...
    try {
        ...
        alert("PDF generate karne mein problem aayi. Dobara try karo ya desktop pe check karo.");
    } finally {
