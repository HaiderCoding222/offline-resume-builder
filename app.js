...
btnDownloadPDF.addEventListener('click', async () => {
    ...
    // Special handling for Blue template (most problematic)
    let scale = 2.0;
    let width = 794;
    
    if (cvElement.classList.contains('cv-blue-container')) {
        scale = 2.1;  // Slightly higher for blue
        width = 794;
    }
    ...
    const opt = {
        ...
        html2canvas: { 
            ...
            allowTaint: true,
            width: width,
            height: Math.max(cvElement.offsetHeight, 1123),
            scrollY: 0,
            scrollX: 0,
            windowWidth: 794
        },
